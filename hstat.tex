...
Managing this hierarchy is often seen as a programming burden, but exposes more robust algorithms than are available otherwise.
...
preconditioned by 1-level Schwarz methods with subdomains of size $H$ (see \cite{smith1996domain}) becomes apparent.
Indeed, we have found low-fill incomplete factorization to be nearly unusable as part of a 1-level additive Schwarz method for problems with slippery beds or steep geometry, even at quite low resolution, as investigated in Section~\ref{sec:1level}.
...
\subsection{Matrices}
The most expensive operations are Jacobian assembly and sparse matrix kernels.  The former involves evaluation of fractional powers and quadrature loops.  While fractional powers take most of the time for residual evaluation, they are less significant than quadrature loops for assembly.  The quadrature loops were explicitly vectorized using SSE2 intrinsics which led to a 30\% speedup on Core 2 and Opteron architectures using both GNU and Intel compilers.  There was no manual vectorization for the Blue Gene/P results quoted in Section~\ref{sec:pscaling}.
...
\begin{table}
  ...
  \begin{tabular}{l|c|c|c|c|c|c}
    \multirow{2}{*}{\backslashbox{kernel}{format}} & \multicolumn{3}{c|}{Core 2, 1 thread} & \multicolumn{3}{c}{Opteron, 4 processes} \\
...
The first, which we use in almost all of our numerical examples, is to rediscretize the system on the coarse mesh.
...
The Galerkin procedure is an alternative which is mandatory for algebraic multigrid.
...
These operators work very well for some problems, but computing the sparse matrix product in parallel involves a significant amount of communication and irregular memory access, so it is relatively expensive.
...
% Would be good to have a curve with cheaper coarse problems/solvers, it sounds like Aron will be able to run those on Wednesday.
...
\begin{table}
  \centering\caption{Average number of GMRES iterations per Newton for field-split preconditioners with different ways of combining the splits and different solvers within the splits.
...
A good geometric multigrid for this problem uses 4-levels with a coarse grid of $10\times 10\times 2$ elements which is semi-refined twice by a factor of 2 in the horizontal, then by a factor of 6 in the vertical to reach the target $40\times 40\times 12$ grid.
The smoothers consist of a domain decomposition method and a subdomain solver which may be exact or inexact.
Direct solves for the subdomain problems on level 1 are very cheap and tend to improve robustness so we always use a direct solve.
...
The coarse levels are smaller in this case so it is more efficient provided the iteratation counts are similar.
Table~\ref{tab:xmg} explores a variety of multigrid preconditioner configurations with each coarse level.
A distinct affect is that inexact subdomain solvers cause minimal performance degradation, cf. \ref{tab:1level} where a factor of 5 to 10 degradation is visible.
...
Finally, we remark that when using grid sequencing and the method in the last row of Table~\ref{tab:xmg}, the problem can be solved in 7 Newton iterations on the fine level and an average of 5.4 V-cycles per Newton.
If the Eisenstat-Walker method is used to avoid oversolving, it takes 8 Newton iterations with a total of 12 V-cycles.
...
This geometric multigrid method demonstrates ``textbook multigrid efficiency'' for extreme topography and basal conditions, and offers $\bigO(1000)$ speedups relative to Picard linearization and 1-level domain decomposition with incomplete factorization which is currently used to solve these equations~\cite{seacism,issm,johnson2007modeling,desmedt2010using,pattyn2003ntd}.
