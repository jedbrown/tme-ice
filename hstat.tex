...
The hydrostatic equations were introduced by \cite{blatter1995vas} as a model of intermediate complexity between the full non-Newtonian Stokes system and the integrated SIA and SSA models.
...
These tridiagonal systems become singular as basal friction $\beta^2 \to 0$, and furthermore, SIA is physically inconsistent if $\beta^2$ is discontinuous even when bounded below by a sufficiently large constant due to discontinuous velocities \citep{fowler2001modelling}.
...
The matrix contribution from discrete horizontal derivatives relative to vertical derivatives is of order $h_z^2/h_x^2$ and thus small when the horizontal grid spacing is large, therefore the hydrostatic matrix becomes similar to the block tridiagonal system representing SIA.
When the bed is very slippery, there are medium to long wavelength modes with vanishing energy in the norm induced by the (nearly singular) hydrostatic equations, but non-vanishing energy in the norm induced by the hydrostatic equations, and these modes are problematic if the tridiagonal SIA system is used to precondition hydrostatic because it will take many iterations for the Krylov solver to locate all of these modes not controlled by the preconditioner.
...
\begin{figure}
  \includegraphics[width=0.8\textwidth]{figures/x-shear}
...
The methods in this paper can efficiently solve the discrete hydrostatic equations in regimes with basal topography and stickyness distribution as extreme as anywhere of interest on earth, far in excess of where the continuum hydrostatic approximation is valid as an approximation to the Stokes equations according to asymptotic analysis~\citep{schoof2010thin} and numerical comparison~\citep{hindmarsh2004numerical,pattyn2008beh}.
Despite being more complete than models involving only a vertically integrated elliptic solve \citep[\eg][]{bueler2009shallow,goldberg2009grounding,goldberg2010variational}, the hydrostatic equations are not suitable for steep outlet glaciers like Jakoshavn Isbr{\ae} or Pine Island Glacier where solution of Stokes problems appears to be unavoidable.

A scalable solver has not yet been proposed for these vertically integrated elliptic problems so the present model is faster than existing implementations at sufficiently high resolution, thus we estimate relative costs with the assumption that a similarly optimal solver exists for the integrated system.
In the \cite{bueler2009shallow} model, the depth dependent velocity components have been factored out of the vertically-integrated nonlinear problem, thus the nonlinear elliptic solve can be completed without revisiting the 3D velocity field.
This is only possible if the constitutive relation can be factored and the formal order of accuracy is lower than the ``hybrid'' model of \cite{goldberg2010variational} which requires reevaluating effective viscosity in 3D and integrating to produce depth-averaged viscosity.
Evaluating effective viscosity accounts for most of the cost of residual evaluation for the hydrostatic equations so we suppose that it's cost is roughly equivalent to the nonlinear evaluation required in a Picard iteration for the hybrid model.
...
If the hybrid equations were instead solved by Newton iteration and the linear solves required similar or less time than evaluating viscosity, then the hybrid equations might be solved as much as 5 times faster than hydrostatic.
